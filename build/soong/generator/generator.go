...
func init() {
	android.RegisterModuleType("chidori_generator", GeneratorFactory)
...
func (g *Module) GenerateAndroidBuildActions(ctx android.ModuleContext) {
	...
	if depRoot == "" {
		...
		depRoot = chidoriExpandVariables(ctx, depRoot)
	}

	// Glob dep_files property
	for _, dep_file := range g.properties.Dep_files {
		dep_file = chidoriExpandVariables(ctx, dep_file)
	...
	cmd := chidoriExpandVariables(ctx, String(g.properties.Cmd))
